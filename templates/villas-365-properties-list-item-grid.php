			...
			<div class="row">
				<div class="col-12 mb-2">
					<div class="_villas-365-property-image">
						...
						<?php endif; ?>
						<?php if($showDiscountLabel === true && Helpers365::CheckObjectPropertiesExist($property, ["discounttexts"], false) && !is_null($property->discounttexts) && is_array($property->discounttexts) && (count($property->discounttexts) > 0)) :
							$discountText = null;
						...
						?>
						<a href="<?php echo (isset($discountsPageURL) && !is_null($discountsPageURL) ? esc_html($discountsPageURL) : Helpers365::MakePropertyURL($propertyPageURL, $property->slug)); ?>" class="_villas-365-property-info-label _villas-365-property-discount"<?php echo $villas365PropertiesOpenInNewTabsHTML; ?>>
						...
						<?php elseif($showFeaturedLabel === true && Helpers365::CheckObjectPropertiesExist($property, ["isfeatured"], false) && !is_null($property->isfeatured) && ((is_bool($property->isfeatured) && ($property->isfeatured === true)) || ($property->isfeatured == 1))) : ?>
						...
						<div class="row h-100">
							<div class="col-12">
								<div class="row">
									...
									<?php if((!isset($discountsOnly) || !$discountsOnly) && !is_null($property->brief) && (trim($property->brief) !== "") && (!isset($hideBrief) || !$hideBrief)) : ?>
									<div class="col-12">
										<div class="_villas-365-property-summary">
											<?php echo esc_html(Helpers365::StrLimit(Helpers365::Get365TranslationValue($property, ["brief"], ["languages", "brief"]), 250)); ?>
										</div>
									</div>
									<?php elseif(isset($discountsOnly) && $discountsOnly && Helpers365::CheckObjectPropertiesExist($property, ["discounttexts"], false) && !is_null($property->discounttexts) && is_array($property->discounttexts) && (count($property->discounttexts) > 0)): ?>
									<div class="col-12">
										<div class="_villas-365-property-summary">
											<?php
											$discountNumber = count($property->discounttexts);
											foreach($property->discounttexts as $key => $discount)
											{
												echo "<strong>" . esc_html($discount) . "</strong>";

												if(Helpers365::CheckObjectPropertiesExist($property, ["discountdescription"], false) && !is_null($property->discountdescription) && is_array($property->discountdescription) && (count($property->discountdescription) > 0) && array_key_exists($key, $property->discountdescription) && !is_null($property->discountdescription[$key]))
												{
													echo "<br><span>" . esc_html(Helpers365::Get365TranslationValue($property->discountdescription[$key], ["en"], [])) . "</span>";
												}

												if($discountNumber > 1 && ($key != ($discountNumber - 1)))
												{
													echo "<br><br>";
												}
											}
