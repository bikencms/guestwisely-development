...
function initMap()
{
	if(typeof(google) !== "undefined")
	{
		...
		for (var count = 0; count < totalProperties; count++)
		{
			...
			{
				let currentMarker = createMarker(_map, _properties[count].latitude, _properties[count].longitude, _properties[count].title, _properties[count].content, _boundary);
				if(firstMarker === null)
				{
		...
		openMarkerInfoWindow(firstMarker);
	}
}
